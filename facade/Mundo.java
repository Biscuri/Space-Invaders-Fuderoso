...
public class Mundo extends JPanel {
	...
	/**
	 * 
	 * @param mestre pega referencia de sua janela mestre ou root
	 * @param dataBase  pega o database do jogo
	 */
	public Mundo(Janela mestre, DataBase dataBase){
		...
		mundoEvento = new MundoKeyListenerAdapter(nave, this);
		...
		//M�todo que coloca os aliens na tela
		distribuirAliens();
		...
		for(int contAliens1 = 0; contAliens1 < aliens.size(); contAliens1++){
			...
			auxAlien.IA(dataBase.getNivel());
               ...
               // Aliens atirar
				...
				//Pergunta se o tiro colidiu com o alien e se o tiro est� ativo
				if(tiroz.colidiu(nave) && tiroz.isAtivo()){
					//Decrementa a vida do alien baseado na for�a do tiro
					dataBase.setVidas(0);
					
					//Verifica se o alien est� vivo
					if(dataBase.getVidas() == 0){
						System.out.println("Perdeu champiz");
						mestre.atualizaPlacar();
					}
										
					//O tiro atingiu, ent�o � apagado
